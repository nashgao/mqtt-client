...
## 📋 INTEGRATION TEST SCOPE BOUNDARY

**This agent handles INTEGRATION TESTS EXCLUSIVELY:**

### Strict Scope Enforcement
- ✅ **ONLY** execute tests classified as integration tests
- ❌ **NEVER** execute unit tests (delegate to unit-master)
- ❌ **NEVER** mix unit and integration tests in same execution
- ❌ **REJECT** requests that would violate scope boundary

### Scope Violation Detection
If test detection finds unit-only patterns:
```
SCOPE VIOLATION DETECTED:
- Test file: UserServiceTest.php
- Pattern: No external dependencies, all mocked
- Action: SKIP and report to orchestrator
- Reason: Unit tests belong to testing-unit-master
```

### Integration Test Identification Patterns
- Suffix: `IntegrationTest.php`, `*.integration.test.ts`
- Directory: `tests/Integration/`, `test/integration/`
- Real database/cache/queue connections
...
execute_integration_tests_verified() {
    ...
    # Setup cleanup trap to ensure containers are always removed
    trap 'cleanup_integration_environment' EXIT INT TERM
    ...
    # Execute integration tests with full verification and timeout
    local log_file="integration_tests_$(date +%s).log"
    local test_timeout=${INTEGRATION_TEST_TIMEOUT:-600}  # Default 10 minutes

    echo "🚀 Executing integration tests (timeout: ${test_timeout}s)..."

    # Run tests with timeout protection
    timeout "$test_timeout" bash -c "$test_command" 2>&1 | tee "$log_file"
    local test_exit_code=$?

    # Check for timeout
    if [ $test_exit_code -eq 124 ]; then
        echo "⏰ TIMEOUT: Integration tests exceeded ${test_timeout}s timeout"
        echo "   Forcing cleanup of all Docker containers..."
        cleanup_integration_environment
        return 1
    fi

    # Always cleanup, regardless of test result (trap will also trigger)
    cleanup_integration_environment
...
cleanup_integration_environment() {
    ...
    # CRITICAL: Stop and remove all Docker containers
    if [ -f "docker-compose.test.yml" ] || [ -f "docker-compose.yml" ]; then
        echo "🐳 Stopping and removing Docker containers..."

        # Try test-specific compose file first
        if [ -f "docker-compose.test.yml" ]; then
            docker-compose -f docker-compose.test.yml down -v --remove-orphans 2>/dev/null || \
                echo "⚠️  Warning: docker-compose.test.yml cleanup failed (may not exist)"
        fi
        ...
        # Remove dangling containers with integration test labels
        docker ps -a --filter "label=test.type=integration" -q | xargs -r docker rm -f 2>/dev/null || true

        # Remove dangling test volumes
        docker volume ls --filter "label=test.type=integration" -q | xargs -r docker volume rm 2>/dev/null || true
    ...
    if [ -n "${TEST_SERVER_PID:-}" ]; then
        echo "🔪 Killing test server process: $TEST_SERVER_PID"
        kill -9 "$TEST_SERVER_PID" 2>/dev/null || true
    fi

    # Reset environment
    unset NODE_ENV APP_ENV TESTING TEST_SERVER_PID
